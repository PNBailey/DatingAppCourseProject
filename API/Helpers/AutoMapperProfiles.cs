...
namespace API.Helpers
{
    public class AutoMapperProfiles : Profile // When using automapper, we must inherit from Profile base class 
    {
        public AutoMapperProfiles()
        {
            ...
            .ForMember(dest => dest.Age, opt => opt.MapFrom(src => src.DateOfBirth.CalculateAge())); // Rather than calculate the age of the user within the Appuser class (this is not very effecient), we calculate the age here when the AppUser is converted to a Member Dto by automapper 
