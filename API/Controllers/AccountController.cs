...
namespace API.Controllers
{
    public class AccountController : BaseApiController
    {
        ...
        public AccountController(UserManager<AppUser> userManager, SignInManager<AppUser> signInManager, ITokenService tokenService, IMapper mapper) // We have to inject a dependancy to the DataContext as we want to get some data from the database from within this controller. With this private property above and this constructor with dependancy injection, we have access to the database via the DbContext just by using _context. We inject our token service as we want to generate a token for our user within this class. As we are now using Identity, we pass in the UserManager as this allows us to access the users in our database. We also pass in the SignInManager
        {
        ...
        { // We use the ActionResult as this is the normal thing we return from any of our controller methods. This is an async method. We specify the type of thing we are returning, in this case an Appuser. 
            ...
            var result = await _userManager.CreateAsync(user, registerDto.Password); // Now that we are using Identity, we create a user this way. Using the CreateAsync method on the _userManager means that the changes will automatically be saved so there is no need to call SaveChangesAsync. Saving the result of this method call will tell us whether it was successful or not. Identity has has a type called IdentityResult and this tells us whether the request was successful. See below
        ...
        {
            return await _userManager.Users.AnyAsync(x => x.UserName == userName.ToLower()); // This method simply checks if the userName is found in our database. We convert the userName to lower case as when we send the username to the database, it will also be in lower case so it ensures it is comparing the strings in the same format
        }
    }
}